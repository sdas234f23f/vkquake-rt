...
foreach ($sub in @("textures","progs","mdl_skins")) { if (Test-Path "$s\$sub") { New-Item -ItemType Directory -Path "$g\$sub" -Force | Out-Null; Copy-Item -Path "$s\$sub\*" -Destination "$g\$sub" -Recurse -Force } }
