...
- **Button/lamp brightness re-tuned** — `+0basebtn`/`+abasebtn` 0.5 → 0.2, `tlight02` 0.5 → 0.8, `tlight07` 0.6 → 0.8, `tlight11` 0.7 → 0.5, `basebutn3` is now a light (`is_light: true`, brightness 0.1) so red/blue buttons don't wash out their surroundings while lamps read correctly.
- **Lightning bolt glows** — `progs/bolt.mdl:frame0` and `progs/bolt1.mdl:frame0` (the lightning gun bolt models) now have an emissive mask (`mdl_skins/bolt_skin0.png`) with pale-blue `ccccff` light, replacing the old `force_rasterize`/`is_light: false` entry that left the bolt invisible to the ray tracer.
...
- **Missing luma masks restored** — `materials.yaml` pointed at six emissive textures that were never committed (`textures/{tlight01,tlight02,tlight07,tlight10,tlight11,rune2_3}_luma.png`), so those lamps lost their emissive/TAL light on a clean checkout while an old build directory kept stale copies alive. All six are now in `vkpt/Source/textures/` and every emissive reference resolves; `textures/tlight02` also pointed at `tlight10_luma.png` (the two files are byte-identical) and now points at its own mask. `z_exit_luma.png` was likewise generated as an albedo-brightness threshold, leaving a level-64 plate across the whole panel — it is now strictly binary and covers only the letter glyphs.

### Changed
- **Second diffuse bounce enabled by default** — `rt_indir2bounces` now defaults to 1 (matches Q2RTX, whose `indirSecondBounce` is on). The indirect pass takes a second *diffuse* bounce off the first bounce hit and gathers `(emission + directLight) * albedo` there, so emissive surfaces (lava) get a second chance to be hit by a ray and spread their color through the scene. Disable with `rt_indir2bounces 0`.
